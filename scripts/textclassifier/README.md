# Baseline Clarifai Runner

A minimal Clarifai local runner model that simply echoes inputs and provides
introspection about its own callable endpoints (`predict`, `generate`, `stream`).


# Baseline Clarifai Runner (Dockerized)

A shareable, minimal Clarifai Local Runner implementation that echoes input, provides
...
Built from a simplified baseline derived from a richer video/AWS example, now trimmed for clarity and educational use.
