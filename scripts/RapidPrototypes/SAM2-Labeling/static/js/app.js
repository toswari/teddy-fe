document.addEventListener('DOMContentLoaded', () => {
    ...
    const state = {
        ...
        currentSegment: 0,
        totalSegments: 0,
        segmentPlayers: {} // {segmentIdx: {obj_id: {points, labels}}}
    };
    ...
    function handleUploadSuccess(response) {
        ...
        state.totalSegments = response.total_segments;
        ...
        // Hide segment navigation buttons initially
        let navDiv = document.getElementById('segment-nav-buttons');
        if (navDiv) navDiv.classList.add('d-none');
        // Show and enable Start Target Selection button and controls
        elements.startPlayerSelection.classList.remove('d-none');
        elements.startPlayerSelection.disabled = false;
        elements.nextPlayer.classList.remove('d-none');
        elements.nextPlayer.disabled = true;
        elements.endPointCollection.classList.remove('d-none');
        elements.endPointCollection.disabled = true;
        elements.processingButtons.classList.add('d-none');
        startSegmentAnnotation(0);
    }

    // Start annotation for a segment
    function startSegmentAnnotation(segmentIdx) {
        state.currentSegment = segmentIdx;
        // Always reset state for new segment
        state.selectingPoints = false;
        state.currentPlayer = null;
        state.canvasPoints = [];
        // Clear masks and points for all players
        Object.keys(state.players).forEach(objId => {
            state.players[objId].points = [];
            state.players[objId].labels = [];
            if (state.players[objId].maskImage) delete state.players[objId].maskImage;
        });
        // Prefill points from last mask of previous segment
        fetch(`/get_prefill_points?session_id=${state.sessionId}&segment_idx=${segmentIdx}`)
          .then(res => res.json())
          .then(prefill => {
            Object.keys(prefill).forEach(objId => {
                if (state.players[objId]) {
                    state.players[objId].points = (prefill[objId].points || []).map(([x, y], idx) => ({
                        normX: x,
                        normY: y,
                        type: 'positive',
                        pointId: idx
                    }));
                    state.players[objId].labels = prefill[objId].labels || [];
                }
            });
            // After prefill, update state.canvasPoints so points are drawn
            state.canvasPoints = [];
            Object.keys(state.players).forEach(objId => {
                (state.players[objId].points || []).forEach((pt, idx) => {
                    state.canvasPoints.push({
                        x: pt.normX * state.canvasWidth,
                        y: pt.normY * state.canvasHeight,
                        type: pt.type,
                        playerId: parseInt(objId),
                        pointId: pt.pointId !== undefined ? pt.pointId : idx
                    });
                });
            });
            // Get first frame
            fetch(`/get_segment_frame?session_id=${state.sessionId}&segment_idx=${segmentIdx}`)
              .then(res => res.json())
              .then(data => {
            ...
            // Get known objects
            fetch(`/get_players?session_id=${state.sessionId}`)
              .then(res => res.json())
              .then(data => {
                // Show per-object annotation UI
                showPerObjectAnnotationUI(data.players);
                elements.annotationStatus.textContent = `Annotate Segment ${segmentIdx + 1} / ${state.totalSegments}`;
            ...
            // Show segment navigation buttons
            showSegmentNavigationButtons();
            // Always reset and show progress bar for every segment
            elements.trackingProgress.classList.remove('d-none');
            elements.progressBar.style.width = '0%';
            elements.progressBar.setAttribute('aria-valuenow', 0);
            elements.progressBar.textContent = '0%';
            updateStatusLine('Ready to track this segment', 'info');
            // elements.resultsArea.classList.add('d-none'); // Do NOT hide results panel between segments
          });
    }

    function updateProcessingButtonsState() {
        // Enable if at least one player has at least one point
        const hasPoints = Object.values(state.players).some(
            player => player.points && player.points.length > 0
        );
        elements.processingButtons.classList.toggle('d-none', !hasPoints);
        elements.generateMasks.disabled = !hasPoints;
        elements.trackAnnotate.disabled = !hasPoints;
    }

    function showPerObjectAnnotationUI(players) {
        // Clear players list
        elements.playersList.innerHTML = '';
        elements.noPlayersMessage.classList.add('d-none');
        // For each player, add a button to start annotating points
        players.forEach(player => {
            addPlayerItem(player.obj_id, player.color);
        });
        // Add event listeners to select-player buttons
        document.querySelectorAll('.select-player').forEach(btn => {
            btn.disabled = false;
            btn.onclick = function() {
                const playerId = parseInt(this.closest('.player-item').dataset.playerId);
                selectPlayerForSegment(playerId);
            };
        });
        // Show instructions
        elements.annotationInstructions.classList.remove('d-none');
        // Hide current player points and point type selection
        elements.currentPlayerPoints.classList.add('d-none');
        elements.pointTypeSelection.classList.add('d-none');
        // Reset button states
        elements.startPlayerSelection.classList.remove('d-none');
        elements.startPlayerSelection.disabled = players.length > 0; // Only enable if no players yet
        elements.nextPlayer.classList.remove('d-none');
        elements.nextPlayer.disabled = players.length === 0;
        elements.endPointCollection.classList.remove('d-none');
        elements.endPointCollection.disabled = true;
        // Enable processing buttons if ready
        updateProcessingButtonsState();
    }

    function selectPlayerForSegment(playerId) {
        // Set current player and enable point selection
        ...
        updatePointsList();
        // Update status
        elements.annotationStatus.textContent = 'Annotating points for Target ' + playerId;
    ...
    // Show segment navigation buttons only after first segment annotation is done
    function showSegmentNavigationButtons() {
        let navDiv = document.getElementById('segment-nav-buttons');
        if (!navDiv) {
            navDiv = document.createElement('div');
            navDiv.id = 'segment-nav-buttons';
            navDiv.className = 'my-3';
            elements.annotationArea.appendChild(navDiv);
        }
        // Only show after first segment is annotated (not on initial load)
        if (state.currentSegment === 0 && !Object.values(state.players).some(p => (p.points && p.points.length > 0))) {
        ...
        continueBtn.textContent = 'Continue to Next Segment';
        continueBtn.onclick = () => saveAndTrackSegment(false);
        navDiv.appendChild(continueBtn);
        const finishBtn = document.createElement('button');
        finishBtn.className = 'btn btn-danger';
        finishBtn.textContent = 'Finish Annotation';
        finishBtn.onclick = () => saveAndTrackSegment(true);
        navDiv.appendChild(finishBtn);
    }

    function saveAndTrackSegment(isFinish) {
        // Gather all points/labels for each object
        const annotations = {};
        Object.keys(state.players).forEach(objId => {
            // Filter out invalid points
            const validPoints = (state.players[objId].points || []).filter(pt => {
                return pt.normX !== null && pt.normY !== null &&
                       pt.normX !== undefined && pt.normY !== undefined &&
                       !isNaN(pt.normX) && !isNaN(pt.normY);
            });
            const pts = validPoints.map(pt => [pt.normX, pt.normY]);
            const lbls = validPoints.map(pt => pt.type === 'positive' ? 1 : 0);
            annotations[objId] = {
                points: pts,
                labels: lbls
            };
        });
        fetch('/save_segment_annotations', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                session_id: state.sessionId,
                segment_idx: state.currentSegment,
            ...
            elements.progressBar.setAttribute('aria-valuenow', 0);
            elements.progressBar.textContent = '0%';
            updateStatusLine('Tracking segment...', 'info');
            fetch('/track_segment', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
              ...
              .then(data => {
                // Poll for progress until segment is done
                startProgressPolling(() => {
                    // After tracking is complete, reset UI for next segment or finish
                    elements.trackingProgress.classList.add('d-none'); // Hide progress bar
                    if (isFinish || state.currentSegment + 1 >= state.totalSegments) {
                        finishAndCombine();
                    } else {
                        // Reset UI for next segment
                        startSegmentAnnotation(state.currentSegment + 1);
                        // Set status to ready for new segment
                        elements.annotationStatus.textContent = `Annotate Segment ${state.currentSegment + 2} / ${state.totalSegments}`;
                        elements.annotationStatus.classList.remove('bg-info', 'bg-warning', 'bg-success', 'bg-danger');
                        elements.annotationStatus.classList.add('bg-info');
                        updateStatusLine('Ready for next segment', 'info');
                    }
                });
              });
        });
    ...
    function finishAndCombine() {
        fetch('/finish_annotation', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ session_id: state.sessionId })
        }).then(() => {
            fetch('/combine_segments', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ session_id: state.sessionId })
            }).then(res => res.json())
              .then(data => {
                ...
                state.downloadUrl = `/download/${state.sessionId}`;
    ...
    function generateMaskForCurrentPlayer() {
        ...
        fetch('/generate_mask_for_player', {
            ...
            body: JSON.stringify({
                session_id: state.sessionId,
                obj_id: playerId,
                segment_idx: state.currentSegment || 0  // Pass current segment index
            })
        })
        ...
        .then(data => {
            ...
            if (data.success) {
                // Verify point count matches before proceeding
                const currentPointCount = state.players[playerId].points.length;
                if (data.point_count !== currentPointCount) {
                    console.log(`Point count mismatch - Server: ${data.point_count}, Client: ${currentPointCount}. Syncing state...`);
                    
                    // Fetch current points from server to sync state
                    fetch('/get_players?session_id=' + state.sessionId)
                    .then(response => response.json())
                    .then(playerData => {
                        const serverPlayer = playerData.players.find(p => p.obj_id === playerId);
                        if (serverPlayer) {
                            // Update local state with server state
                            state.players[playerId].points = serverPlayer.points;
                            state.canvasPoints = state.canvasPoints.filter(p => p.playerId !== playerId);
                            serverPlayer.points.forEach((point, idx) => {
                                state.canvasPoints.push({
                                    x: point.normX * state.canvasWidth,
                                    y: point.normY * state.canvasHeight,
                                    type: point.type,
                                    playerId: playerId,
                                    pointId: idx
                                });
                ...
                maskImage.onload = () => {
                    ...
                    // Update status
                    elements.annotationStatus.textContent = "Mask updated";
                    elements.annotationStatus.classList.remove('bg-warning');
                    elements.annotationStatus.classList.add('bg-success');
                };
                maskImage.src = data.preview;
            } else {
                console.error('Mask generation failed:', data.error);
        ...
        .catch(error => {
            ...
            elements.annotationStatus.textContent = "Error generating mask";
            elements.annotationStatus.classList.remove('bg-warning');
            elements.annotationStatus.classList.add('bg-danger');
        });
    }
    ...
    function endPointCollection() {
        ...
        elements.endPointCollection.disabled = true;
        // Update status
        elements.annotationStatus.textContent = 'Select another object or continue.';
        elements.annotationStatus.classList.remove('bg-success', 'bg-warning', 'bg-danger');
        elements.annotationStatus.classList.add('bg-info');
        // Show generate mask button
    ...
    function updatePointsList() {
        ...
        // Update badge
        const badge = document.querySelector('#current-player-points .badge');
        if (badge) {
        ...
        // Update the Selected Targets summary for all players
        Object.keys(state.players).forEach(objId => {
            const playerItem = document.querySelector(`.player-item[data-player-id="${objId}"]`);
            if (playerItem) {
                const pointsCount = (state.players[objId].points || []).length;
                const pointsCountElem = playerItem.querySelector('.player-points-count');
                if (pointsCountElem) {
                    pointsCountElem.textContent = `${pointsCount} points`;
                }
            }
        });
    }
    ...
    function trackAndAnnotate() {
        console.log("Starting track and annotate process...");
        // Update UI
        ...
        console.log("Making API call to /track_and_annotate");
        // Make API call
        ...
        .then(data => {
            console.log("Track and annotate response:", data);
            if (data.success) {
                console.log("Starting progress polling...");
                // Start polling for progress
                startProgressPolling(() => {
                    // After processing is complete, advance to next segment if available
                    if (state.currentSegment + 1 < state.totalSegments) {
                        startSegmentAnnotation(state.currentSegment + 1);
                    } else {
                        // If no more segments, show results area
                        elements.resultsArea.classList.remove('d-none');
                        state.downloadUrl = `/download/${state.sessionId}`;
                        elements.downloadResults.disabled = false;
                    }
                ...
                console.error("Track and annotate failed:", data.error);
                // Update UI on error
        ...
        .catch(error => {
            ...
            elements.annotationStatus.classList.add('bg-danger');
            // Show error message
            updateStatusLine('Network error occurred: ' + error.message, 'danger');
            // Re-enable button
            elements.trackAnnotate.disabled = false;
        });
    }
    ...
    // Update the progress polling function to accept a callback for completion
    function startProgressPolling(onComplete) {
        ...
        function checkProgress() {
            console.log("Checking tracking progress...");
            fetch(`/check_tracking_progress?session_id=${state.sessionId}`)
                ...
                .then(progressData => {
                    ...
                    if (progressData.status && progressData.status !== lastStatus) {
                        console.log(`Status updated: ${progressData.status}`);
                        updateStatusLine(`Status: ${progressData.status}`, 'info');
                        lastStatus = progressData.status;
                    }
                    // Show tracking results area if not already visible
                    if (elements.trackingResults.classList.contains('d-none')) {
                        elements.trackingResults.classList.remove('d-none');
                    }
                    // Update frame preview if we have a new frame
                    if (progressData.current_frame !== undefined && progressData.current_frame !== lastFrameIdx) {
                        lastFrameIdx = progressData.current_frame;
                        const framePreview = progressData.frame_previews[progressData.current_frame];
                        if (framePreview) {
                            console.log(`Updating preview for frame ${progressData.current_frame}`);
                            // Create new image to force reload
                            const newImage = new Image();
                            newImage.onload = function() {
                                elements.trackingPreview.src = this.src;
                            };
                            newImage.src = framePreview + '?t=' + new Date().getTime();
                            // Update frame counter
                            elements.currentFrame.textContent = 
                                `Frame: ${progressData.current_frame + 1}/${progressData.total_frames}`;
                            // Update slider if available
                            if (elements.frameSlider) {
                                elements.frameSlider.value = progressData.current_frame;
                                elements.frameSlider.max = progressData.total_frames - 1;
                            }
                        }
                    }
                    // Check for the most recent progress update
                    if (progressData.updates && progressData.updates.length > lastUpdateCount) {
                        const newUpdates = progressData.updates.slice(lastUpdateCount);
                        newUpdates.forEach(update => {
                            console.log("New update:", update);
                            updateStatusLine(update.message, 'info');
                        });
                        lastUpdateCount = progressData.updates.length;
                    }
                    ...
                    if (progressData.is_complete) {
                        ...
                        if (progressData.success) {
                            ...
                            if (!sliderInitialized && progressData.total_frames) {
                                console.log(`Initializing frame slider with ${progressData.total_frames} frames`);
                            ...
                            handleProcessingError(progressData.error || 'Unknown error');
                        }
                    }
                })
                .catch(error => {
                    ...
                    if (failedAttempts >= MAX_FAILED_ATTEMPTS) {
                        ...
                        updateStatusLine(`Error checking progress (attempt ${failedAttempts}). Will retry...`, 'warning');
                    }
                });
        }
