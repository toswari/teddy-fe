...
def process_model_prediction(model, inputs, timeout_seconds=6):
    for attempt in range(3):
        try:
            ...
            if status == "success":
                ...
                if attempt == 2:
                    logger.error(f"Model prediction failed after {timeout_seconds}s and 3 attempts")
                    return f"Error: Timeout after {timeout_seconds}s and 3 attempts"
                wait_time = 2 ** attempt
                logger.info(f"Prediction timeout, retry {attempt + 1}/3 after {wait_time}s")
                time.sleep(wait_time)
            else:  # status == "error"
                if attempt == 2:
                    logger.error(f"Model prediction error after 3 attempts: {str(result)}")
                    return f"Error: {str(result)}"
                wait_time = 2 ** attempt
                logger.info(f"Prediction error, retry {attempt + 1}/3 after {wait_time}s: {str(result)}")
                time.sleep(wait_time)
        except Exception as e:
            if attempt == 2:
                logger.error(f"Unexpected error in prediction after 3 attempts: {str(e)}")
                return f"Error: {str(e)}"
            wait_time = 2 ** attempt
            logger.info(f"Unexpected error, retry {attempt + 1}/3 after {wait_time}s: {str(e)}")
            time.sleep(wait_time)

def process_frame(frame, model_config, prompt, previous_frames=None, max_context_frames=5):
    ...
    try:
        ...
        context_text = ""
        if previous_frames:
            # Get the last N frames for context
            recent_frames = previous_frames[-max_context_frames:]
            context_text = "Utilize the results from previous frames to assist in your analysis of this frame. Analyze the frame independently using the previous frames to help understand the action of this frame and the timings. The previous analysis may be incorrect in detail or action, so you should use your own judgement. Previous 5 frames analysis (with frame numbers and timestamps):\n"
            for idx, prev_frame in enumerate(recent_frames):
                if not prev_frame.get('error'):
                    frame_num = len(previous_frames) - len(recent_frames) + idx + 1
                    timestamp = prev_frame.get('timestamp', 0)
                    context_text += f"Frame {frame_num} at {timestamp:.2f}s: {json.dumps(prev_frame)}\n"
...
def extract_frames(video_path, fps=1):
    ...
    try:
        ...
        logger.info(f"Video properties - Total frames: {total_frames}, FPS: {video_fps}, Duration: {duration}")
        ...
        while True:
            ...
            if frame_count % frame_interval == 0:
                ...
                logger.debug(f"Extracted frame {frame_count} at timestamp {frame_count/video_fps}")
...
def analyze_video(video_path, model_config, prompt, stream=False):
    ...
    try:
        ...
        for i, (frame, timestamp) in enumerate(zip(frames, timestamps)):
            logger.info(f"Processing frame {i+1}/{len(frames)} at timestamp {timestamp:.2f}s")
            ...
            if not frame_result.get('error'):
                logger.info(f"Frame {i+1} processed successfully: {json.dumps(frame_result)}")
            else:
                logger.warning(f"Error processing frame {i+1}: {frame_result.get('error')}")
...
def generate_summary(frame_results):
    ...
    try:
        ...
        confidences = [r.get("confidence", 0) for r in frame_results if "confidence" in r]
