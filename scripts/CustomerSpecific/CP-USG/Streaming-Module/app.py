...
# Reduce OpenCV videoio noise and set FFmpeg timeouts for network streams
try:
	# Suppress INFO/WARN logs from OpenCV videoio to avoid spam on stop
	if hasattr(cv2, "utils") and hasattr(cv2.utils, "logging"):
		cv2.utils.logging.setLogLevel(cv2.utils.logging.LOG_LEVEL_ERROR)
except Exception:
	pass
# Hint OpenCV's FFmpeg backend to use shorter network timeouts (in microseconds)
os.environ.setdefault(
	"OPENCV_FFMPEG_CAPTURE_OPTIONS",
	"rtsp_transport;tcp|stimeout;5000000|rw_timeout;5000000|timeout;5000000"
)
...
def _host_requires_custom_cert(base_url: Optional[str]) -> bool:
	"""Return True if base_url is set and the hostname is not clarifai.com.*"""
	if not base_url:
		return False
	try:
		parsed = urlparse(base_url)
		host = parsed.hostname or ""
		return not host.endswith("clarifai.com")
	except Exception:
		return True

def _default_cert_store_for_host(hostname: str) -> str:
	base = Path.home() / ".clarifai_certs"
	base.mkdir(parents=True, exist_ok=True)
	return str(base / f"{hostname}.pem")

def ensure_root_certificate(base_url: str) -> Optional[str]:
	"""Ensure a PEM certificate exists for the server behind base_url by fetching it via TLS.

	For non-clarifai.com endpoints, we retrieve the server certificate from the TLS handshake
	and store it in ~/.clarifai_certs/<host>.pem. Returns the path or None on failure.
	"""
	if not _host_requires_custom_cert(base_url):
		return None
	try:
		parsed = urlparse(base_url)
		...
		port = parsed.port or (443 if (parsed.scheme or "https").lower().startswith("https") else 443)
		# Fetch server certificate in PEM format
		pem = ssl.get_server_certificate((host, port))
		if not pem:
			return None
		target_path = _default_cert_store_for_host(host)
...
def list_models(user_id: str, app_id: Optional[str], portal_base_host: Optional[str] = "clarifai.com"):
	...
	try:
		if not user_id:
			return []
		usermodels = []
		# Optional SDK kwargs for custom endpoints
		kwargs = {}
		base_url_env = os.environ.get("CLARIFAI_BASE_URL")
		root_path_env = os.environ.get("CLARIFAI_ROOT_CERTIFICATES_PATH") or os.environ.get("CLARIFAI_ROOT_CERTS_PATH")
		if base_url_env and _host_requires_custom_cert(base_url_env) and root_path_env:
			kwargs = {"base_url": base_url_env, "root_certificates_path": root_path_env}
		if app_id:
			# Create App within try to avoid raising when credentials are missing
			apps_to_check = []
			try:
				apps_to_check.append((app_id, App(user_id=user_id, app_id=app_id, **kwargs)))
			except Exception:
				apps_to_check = []
		else:
			...
			apps_to_check = []
			try:
				for app in User(user_id=user_id, **kwargs).list_apps():
					app_id_item = getattr(app, "app_id", None) or getattr(app, "id", None)
					apps_to_check.append((app_id_item, App(user_id=user_id, app_id=app_id_item, **kwargs)))
			except Exception:
...
def run_model_inference(det_threshold, frame, model_option, color=(0, 255, 0)):
	...
	# If no model_option (decode-only), return original frame with no regions
	if not model_option:
		return frame, None, []
	# Prefer ID-based initialization to honor custom base_url/root certificates
	...
	processed = draw_regions_on_frame(frame, regions, det_threshold, color=color, model_name=(model_option.get('Name') if isinstance(model_option, dict) else None))
...
# Try to initialize Clarifai auth only if some credentials are present; otherwise skip to avoid noisy errors
auth = None
userDataObject = type("UserData", (), {"user_id": "", "app_id": ""})()
_env_has_token = bool(
	os.environ.get("CLARIFAI_PAT")
	or os.environ.get("CLARIFAI_SESSION_TOKEN")
)
_env_has_user = bool(os.environ.get("CLARIFAI_USER_ID"))
if _env_has_token and _env_has_user:
	try:
		auth = ClarifaiAuthHelper.from_streamlit(st)
		try:
			userDataObject = auth.get_user_app_id_proto()
		except Exception:
			# If proto lookup fails, keep empty defaults
			pass
	except Exception:
		# Silent: we'll fall back to manual inputs
		pass

# Allow user to provide custom Clarifai credentials
with st.expander("Authentication (Clarifai)", expanded=False):
	# Prefer values from helper when available; else fall back to envs
	default_user_id = getattr(userDataObject, "user_id", "") or os.environ.get("CLARIFAI_USER_ID", "")
	default_app_id = getattr(userDataObject, "app_id", "") or os.environ.get("CLARIFAI_APP_ID", "")
	...
	if _host_requires_custom_cert(st.session_state.auth_base_url):
		st.caption("Custom base URL detected")
	# For custom hosts, the app will automatically fetch and store the server certificate from the base URL
	col_apply, col_clear = st.columns([1,1])
	with col_apply:
		if st.button("Apply credentials"):
			if not (st.session_state.auth_user_id and st.session_state.auth_pat):
				st.error("User ID and PAT are required. App ID is optional.")
			else:
				os.environ["CLARIFAI_PAT"] = st.session_state.auth_pat
				os.environ["CLARIFAI_USER_ID"] = st.session_state.auth_user_id
				if st.session_state.auth_app_id:
					os.environ["CLARIFAI_APP_ID"] = st.session_state.auth_app_id
				else:
					if "CLARIFAI_APP_ID" in os.environ:
						del os.environ["CLARIFAI_APP_ID"]
				if st.session_state.auth_base_url:
					os.environ["CLARIFAI_BASE_URL"] = st.session_state.auth_base_url
					# If custom base, ensure root certificate is present and set env
					if _host_requires_custom_cert(st.session_state.auth_base_url):
						root_path = ensure_root_certificate(st.session_state.auth_base_url)
						if root_path:
							st.session_state.root_cert_path = root_path
							os.environ["CLARIFAI_ROOT_CERTIFICATES_PATH"] = root_path
							os.environ["CLARIFAI_ROOT_CERTS_PATH"] = root_path
							st.info(f"Using root certificate: {root_path}")
						else:
							st.warning("Custom base URL detected, but server certificate could not be fetched. Verify the URL is reachable on 443.")
				try:
					list_models.clear()
				except Exception:
					pass
	...
	with col_clear:
		if st.button("Clear credentials"):
			for k in ("CLARIFAI_PAT", "CLARIFAI_BASE_URL", "CLARIFAI_USER_ID", "CLARIFAI_APP_ID"):
				if k in os.environ:
					del os.environ[k]
			try:
				list_models.clear()
...
# Ensure a global stop_event exists
if "stop_event" not in st.session_state:
	st.session_state.stop_event = threading.Event()
if "stop_requested" not in st.session_state:
	st.session_state.stop_requested = False

# Single global Stop button (always enabled so you can stop a run even if state wasn't updated yet)
if st.button("Stop Processing", type="secondary", key="stop_processing_global"):
	st.session_state.stop_event.set()
	st.session_state.stop_requested = True
	st.session_state.processing_active = False

# If a stop was requested in a previous run, ensure flags are consistent
...
if video_option == "Supported Video URLs":
	...
	require_models = not skip_inference
	# Only fetch models if we need them and have credentials
	available_models = []
	if require_models:
		if not has_valid_credentials():
			st.info("Enter Clarifai credentials in the Authentication section above, or enable 'Skip model inference' to proceed without models.")
			st.stop()
	...
	with cols_actions[0]:
		if require_models and not st.session_state.get("processing_active", False) and st.button("Refresh Models"):
			list_models.clear()  # clear cache
			available_models = list_models(user_id, app_id, st.session_state.get("portal_base_host", "clarifai.com"))
	with cols_actions[1]:
		# Stop handled by global button; no per-section stop button
		pass
	...
	if require_models and not available_models:
	...
	if require_models:
		...
		model_options = [None for _ in url_list]

	process_clicked = st.button("Process Videos", disabled=st.session_state.get("processing_active", False))
	if process_clicked:
		# Start a new run with a fresh stop event
		st.session_state.stop_event = threading.Event()
		st.session_state.stop_requested = False
		st.session_state.processing_active = True

	# Only run processing block if explicitly started and not stopped
	if st.session_state.get("processing_active", False) and not st.session_state.stop_event.is_set() and not st.session_state.get("stop_requested", False):
		...
		try:
			...
			def process_video(video_url, index, model_option, stop_event: threading.Event):
				...
				is_http = is_http_url(original_url)
				is_network = is_hls or is_rtsp or is_http

				cap = None
				frame_count = 0
				frames_produced = 0
				start_time = time.time()
				read_timeout = 12.0 if is_network else 6.0
				last_regions = []

				# Strategy:
				# - Local files: try OpenCV first (fast), then fallback to imageio.
				# - HTTP(S) files: try OpenCV first (restores previous working behavior), fallback to imageio.
				# - HLS/RTSP: use imageio first to avoid long OpenCV stop timeouts.
				try_opencv_first = (not is_network) or (is_http and not is_hls and not is_rtsp) or (is_hls or is_rtsp)

				if try_opencv_first:
					# Try OpenCV for local files and HTTP(S) file URLs
					if original_url.startswith("http://"):
						candidates = ["https://" + original_url[len("http://"):], original_url]
					else:
					...
					if cap is not None and cap.isOpened():
						try:
							while cap.isOpened() and not stop_event.is_set():
								...
								if not ret:
									if frame_count == 0:
										msg_queue.put(("error", index, "Source opened but returned no frames. This may be a codec or server issue."))
									break
								frame_ts = time.time()
								try:
									if not skip_inference and (frame_count % frame_skip == 0):
										...
										processed_frame = draw_regions_on_frame(frame, last_regions, det_threshold, model_name=(model_option.get('Name') if isinstance(model_option, dict) else None))
				...
				# Fallback to imageio only if OpenCV didn't produce frames
				if (frames_produced == 0) and not stop_event.is_set():
					try:
						msg_queue.put(("info", index, "Decoding with imageio/ffmpeg..."))
						# Prefer imageio v2 for network so we can set ffmpeg timeouts
						import imageio
						src = original_url
						ff_params = []
						if is_hls or is_rtsp or is_http:
							ff_params = [
							...
							if is_rtsp:
								ff_params += ['-rtsp_transport', 'tcp', '-stimeout', '5000000']
							if is_hls:
								ff_params += ['-fflags', 'nobuffer', '-allowed_extensions', 'ALL', '-headers', 'User-Agent: Mozilla/5.0\r\nAccept: */*\r\n',]
						reader = imageio.get_reader(src, 'ffmpeg', ffmpeg_params=ff_params) if ff_params else imageio.get_reader(src)
						im_idx = 0
						for frm in reader:
							...
							try:
								...
								if not skip_inference and (im_idx % frame_skip == 0):
									...
									processed_frame = draw_regions_on_frame(frame_bgr, last_regions, det_threshold, model_name=(model_option.get('Name') if isinstance(model_option, dict) else None))
								rgb_frame = cv2.cvtColor(processed_frame, cv2.COLOR_BGR2RGB)
								safe_put(frame_queues[index], (frame_ts, rgb_frame))
								msg_queue.put(("frame", index, frame_ts))
							except Exception as inf_err:
						...
						# As last resort, try imageio v3
						try:
							import imageio.v3 as iio
							msg_queue.put(("info", index, "Trying imageio v3 decoder..."))
							im_idx = 0
							for frm in iio.imiter(original_url):
								if stop_event.is_set():
									break
								try:
									frame_ts = time.time()
							...
							msg_queue.put(("error", index, f"All decoders failed: {fb_err}; v3 also failed: {fb2_err}"))
			...
			if st.session_state.stop_event.is_set() or st.session_state.get("stop_requested", False):
				status_placeholder.warning("Stopping processing...")
				st.session_state.processing_active = False
			for t in threads:
				t.join(timeout=0.5)
			# Give readers a short window to exit cleanly after stop to avoid lingering sockets
			deadline = time.time() + 5.0
			while any(t.is_alive() for t in threads) and time.time() < deadline:
				time.sleep(0.1)
			# Ensure we don't auto-restart and mark run finished
			st.session_state.stop_requested = False
			# Reset the stop event for a future run
			st.session_state.stop_event = threading.Event()
			st.session_state.processing_active = False
	...
	# If user doesn't want inference, allow processing without credentials
	require_models = not skip_inference
	...
	# If models are required, ensure credentials and then fetch
	available_models = []
	if require_models:
		if not has_valid_credentials():
			st.info("Enter Clarifai credentials in the Authentication section above, or enable 'Skip model inference' to proceed without models.")
	...
	# Start processing with a fresh state, then run in the same pattern as URLs
	process_clicked_local = st.button("Process Videos", disabled=st.session_state.get("processing_active", False))
	if process_clicked_local:
		st.session_state.stop_event = threading.Event()
		st.session_state.stop_requested = False
		st.session_state.processing_active = True
			...
			def process_video(video_url, index, model_option, stop_event: threading.Event):
						...
						while cap.isOpened() and not stop_event.is_set():
							...
							try:
								if not skip_inference and (frame_count % frame_skip == 0):
									...
									processed_frame = draw_regions_on_frame(frame, last_regions, det_threshold, model_name=(model_option.get('Name') if isinstance(model_option, dict) else None))
								rgb_frame = cv2.cvtColor(processed_frame, cv2.COLOR_BGR2RGB)
								safe_put(frame_queues[index], (time.time(), rgb_frame))
				...
				if frames_produced == 0 and not stop_event.is_set():
					try:
						...
						im_idx = 0
						iterator = iio.imiter(original_url)
						for frm in iterator:
							if stop_event.is_set():
								break
							try:
