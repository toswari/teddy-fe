...
class ComputeLossOTA:
    ...
    def __call__(self, p, targets, imgs, mask=None):  # predictions, targets, model
        device = targets.device
        lcls, lbox, lobj = torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device)
            ...
            if n:
                ...
                if self.nc > 1:  # cls loss (only if multiple classes)
                    ...
                    if mask is not None:
                        ...
                        # For each sample in batch, mask out the loss for masked classes
                        sample_mask = mask[b][:, None, :]  # (n, 1, nc)
                        t = t * sample_mask.squeeze(1)
                        pred = ps[:, 5:] * sample_mask.squeeze(1)
                        lcls += self.BCEcls(pred, t)
                    else:
                        lcls += self.BCEcls(ps[:, 5:], t)  # BCE
            obji = self.BCEobj(pi[..., 4], tobj)
            lobj += obji * self.balance[i]  # obj loss
