...
def convert(sequence, image_size):
    ...
    data = Data()
    data.video.url = sequence['data']['video']
    data.video.video_info.width = mot_df['image_width'].iloc[0]
    data.video.video_info.height = mot_df['image_height'].iloc[0]
...
def main(json_path, out_dir, image_size):
    ...
    for sequence in mot_data:
        ...
        data = convert(sequence, image_size)
