...
# --- Page Configuration ---
st.set_page_config(
    page_title="HelloFresh - AI Brand Compliance Specialist",
    page_icon="✅",
    layout="wide",
)
...
# --- Main Application ---
def main():
    st.title("🎯HelloFresh - AI Brand Compliance Specialist")
    ...
    if st.button("🔍 Analyze Compliance", type="primary", use_container_width=True):
        if uploaded_files:
            ...
            try:
                ...
                # Store validation info for consolidated message
                validation_info = {
            ...
            with st.spinner(f"🚀 Processing {len(all_images)} images using batch API..."):
                ...
                try:
                    ...
                    # Store processing info for consolidated message
                    processing_info = {
            ...
            # Display consolidated success message
            st.success(f"""✅ **Analysis Complete!** 
📁 Validated: {validation_info['image_count']} images ({validation_info['total_size_mb']:.1f} MB)  
⚡ Processed in {processing_info['processing_time']:.1f}s (avg {processing_info['avg_time_per_image']:.1f}s per image)  
📊 Analyzed {len(all_results)} assets successfully!""")
...
def _derive_recommendations_from_violations(violations):
    ...
    for v in violations:
        ...
        if 'typeface' in rule or 'font' in desc.lower():
            suggestions.append("Use mandated 'HelloFresh Brand Font' with proper weight.")
...
def show_detailed_results(all_results):
    ...
    # Remove previous CSS badge injection (kept placeholder for future style toggles)
    if 'hf_css_injected' in st.session_state:
        del st.session_state['hf_css_injected']
    ...
    for result in all_results:
        ...
        # Status emoji - treat "No Logo Found" with 0 violations as compliant
        violations_count = len(result.get('violations', []))
        compliance_status = result.get('compliance_status', 'Unknown')
        
        if compliance_status == 'No Logo Found' and violations_count == 0:
            status_emoji = '✅'  # Green check for no violations
        else:
            status_emoji = {
                'Compliant': '✅',
                'Non-Compliant': '❌',
        ...
        # Display status text - show as compliant when no violations
        if compliance_status == 'No Logo Found' and violations_count == 0:
            display_status = "No Logo Found (Compliant)"
        else:
            display_status = compliance_status
        
        with st.expander(f"{status_emoji} {asset_display_name} - {display_status}"):
            ...
            with col2:
                ...
                with tab1:
                    ...
                    # Check if this is the new HelloFresh-style format
                    if isinstance(json_output, dict) and 'compliance_status' in json_output:
                        # New HelloFresh-style format
                        ...
                        if summary_text:
                            ...
                            elif compliance_status == "No Logo Found":
                                st.info(f"ℹ️ {summary_text}")
                            else:
                                st.info(summary_text)
                        
                        # Display violations if any
                        json_violations = json_output.get('violations', [])
                        if json_violations or violations:
                            # Use JSON violations if available, otherwise fall back to parsed violations
                            display_violations = json_violations if json_violations else violations
                            st.write(f"**Violations count: {len(display_violations)}**")
                            
                            if display_violations:
                                for i, v in enumerate(display_violations, 1):
                                    if isinstance(v, dict):
                                        rule = v.get('rule_violated', 'Unknown Rule')
                                        desc = v.get('description', 'No description provided')
                                        recommendation = v.get('recommendation', '')
                                        
                                        # Display violation with description and recommendation
                                        st.write(f"{i}. **{rule}**: {desc}")
                                        if recommendation:
                                            st.write(f"   *Recommendation: {recommendation}*")
                                    elif isinstance(v, str):
                                        st.write(f"  {i}. {v}")
                            else:
                                st.write("  No violations found - asset meets brand guidelines.")
                        else:
                            st.write("**Violations count: 0**")
                            st.write("  No violations found - asset meets brand guidelines.")
                        
                        # Display confidence score if available
                        confidence = json_output.get('confidence_score')
                        if confidence is not None:
                            st.write(f"**Confidence Score:** {confidence:.2f}")
                    
                    else:
                        # Legacy format handling - check for old HelloFresh 7-key format
                        hf_check_keys = {
                        ...
                        # Try to find HelloFresh format in json_output or summary_text
                        if isinstance(json_output, dict) and any(k in json_output for k in hf_check_keys):
                            parsed_hf = json_output
                        elif isinstance(summary_text, dict) and any(k in summary_text for k in hf_check_keys):
                            parsed_hf = summary_text
                        elif isinstance(summary_text, str) and summary_text.strip().startswith('{'):
                            try:
                                possible = json.loads(summary_text)
                                if isinstance(possible, dict) and any(k in possible for k in hf_check_keys):
                                    parsed_hf = possible
                            except Exception:
                                parsed_hf = None

                        if parsed_hf:
                            # Old HelloFresh 7-key format
                            passed_labels = []
                            violations_list = []

                            for key, label in hf_check_keys.items():
                                data = parsed_hf.get(key)
                                if not isinstance(data, dict):
                                    continue
                                met = bool(data.get('met'))
                                rec = data.get('recommendation') or data.get('description') or ''
                                if met:
                                    passed_labels.append(label)
                                else:
                                    violations_list.append({
                                        'rule_violated': label,
                                        'recommendation': rec or 'No recommendation provided.'
                                    })
                            
                            passed_line = 'Passed Checks: '
                            if passed_labels:
                                segments = [f"✅ {lbl}" for lbl in passed_labels]
                                passed_line += ' | '.join(segments)
                            else:
                                passed_line += 'None'
                            st.write(passed_line)

                            if violations_list:
                                st.write(f"Violations count: {len(violations_list)}")
                                for i, v in enumerate(violations_list, 1):
                                    st.write(f"  {i}. {v['rule_violated']}: {v['recommendation']}")
                            else:
                                st.write("Violations count: 0")
                                st.write("  No violations found - asset meets brand guidelines.")
                        else:
                            ...
                            elif 'non-compliant' in summary_text.lower() or 'violation' in summary_text.lower():
                                st.warning(summary_text)
                            elif 'compliant' in summary_text.lower() or 'meets' in summary_text.lower():
                                st.success(summary_text)
                            else:
                                st.info(summary_text)
                            
                            # Show violations if any were parsed
                            if violations:
                                st.write(f"**Violations count: {len(violations)}**")
                                for i, v in enumerate(violations, 1):
                                    if isinstance(v, dict):
                                        rule = v.get('rule_violated', 'Unknown Rule')
                                        desc = v.get('description', v.get('recommendation', 'No description provided'))
                                        st.write(f"  {i}. **{rule}**: {desc}")
                                    elif isinstance(v, str):
                                        st.write(f"  {i}. {v}")
                            else:
