        ...
        for labeler in labelers:
            ...
            for concept in concepts:
                for label_type in LABEL_TYPES:
                    ...
                    # Generate data for both manual and model-assisted labeling
                    for labeling_type in ["Manual", "Model-Assisted"]:
                        time_factor = (
                            1
                            if labeling_type == "Manual"
                            else np.random.uniform(0.4, 0.6)
                        )
                        data.append(
                            {
                                "Date": date,
                                ...
                                "Quality": quality
                                * (
                                    1 + np.random.uniform(0, 0.05)
                                    if labeling_type == "Model-Assisted"
                                    else 1
                                ),
                                "Annotations": annotations,
                                "Errors": errors,
                                "Error_Rate": (
                                    (errors / annotations) * 100
                                    if annotations > 0
                                    else 0
                                ),
                                "Time_Taken": annotations * (1 / speed) * time_factor,
                            }
                        )
...
tab1, tab2, tab3 = st.tabs(
    [
        "Current State",
        "Future State with Model-Assisted Labeling",
        "Future State with Model-Assisted LabelingV2",
    ]
)

with tab1:
    # Current State Dashboard (existing code)
    st.header("Current Labeling Performance")
...
with tab2:
    st.header("Future State with Model-Assisted Labeling")
    ...
    model_error = manual_error * 0.7  # Assume 30% error reduction
