...
def generate_sample_data(num_days=90):
    ...
    for date in dates:
        for labeler in labelers:
            ...
            improvement_factor = 1 + (num_days - dates.index(date)) / (num_days * 3)
            ...
            for concept in concepts:
                for label_type in LABEL_TYPES:
                    annotations = int(np.random.normal(35, 5) * improvement_factor)
                    ...
                    data.append(
                        {
                            ...
                            "Error_Rate": (
                                (100 - manual_quality) if annotations > 0 else 0
                            ),
                    ...
                    assisted_speed = manual_speed * 2.0  # 2x speed increase
...
if isinstance(date_range, (list, tuple)) and len(date_range) == 2:
    ...
    start_date = end_date = date_range if not isinstance(date_range, (list, tuple)) else df["Date"].min()
...
with tab1:
    ...
    avg_quality = manual_df["Quality"].mean()
    average_error_rate = 100 - avg_quality

    col2.metric("Average Quality Score", f"{avg_quality:.2f}%")
    col3.metric("Average Error Rate", f"{average_error_rate:.2f}%")
    ...
    fig.update_layout(
        xaxis_title="Speed (Annotations per Hour)",
        yaxis_title="Quality (%)",
        showlegend=True,
    )
    ...
    # Key for the scatter plot
    st.markdown(
        """
    **Legend:**
    - **X-axis**: Speed (Annotations per Hour) - Represents the average number of annotations a labeler can complete in an hour.
    - **Y-axis**: Quality (%) - Represents the average quality score of the labeler's annotations.
    - **Bubble Size**: Total Annotations - The size of each bubble represents the total number of annotations completed by the labeler.
    - **Color**: Each color represents a different labeler.

    This scatter plot allows you to visualize the relationship between a labeler's speed and quality, while also showing their overall productivity (total annotations). Ideally, you want to see labelers in the top-right quadrant, indicating high speed and high quality.
    """
    )
...
with tab2:
    ...
    annotation_delta = f"{(model_annotations - manual_annotations) / manual_annotations:.1%}" if manual_annotations > 0 else "N/A"
    col1.metric(
        "Total Annotations",
        f"{model_annotations:,}",
        annotation_delta,
    ...
    model_error = 100 - Average_Quality_Score
    ...
    # Check if both labeling types exist in the filtered data
    model_assisted_time = time_comparison.loc[
        time_comparison["Labeling_Type"] == "Model-Assisted", "Time_Taken"
    ]
    manual_time = time_comparison.loc[
        time_comparison["Labeling_Type"] == "Manual", "Time_Taken"
    ]
    
    if len(model_assisted_time) > 0 and len(manual_time) > 0 and manual_time.values[0] > 0:
        time_savings = (
            1 - model_assisted_time.values[0] / manual_time.values[0]
        ) * 100
        st.metric("Time Reduction", f"{time_savings:.1f}%", "↓")
    else:
        st.metric("Time Reduction", "N/A", "Insufficient data")
    ...
    industry_manual_speed = 50  # 100 annotations/hour
    industry_model_speed = (
        industry_manual_speed * 2.0  # 2x speed increase
    )  # 150 annotations/hour (50% increase)
    ...
    - **Model-Assisted Labeling**: With AI assistance, labelers can annotate approximately {industry_model_speed} items per hour (a 200% increase) 
...
# Footer
st.sidebar.markdown("---")
st.sidebar.markdown("© Clarifai - Labeler Quality Dashboard\nVersion 1.0")
