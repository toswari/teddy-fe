...
def home():
    ...
    username = os.getenv("CLARIFAI_USERNAME", "")
    pat = os.getenv("CLARIFAI_PAT", "")
    return render_template("index.html", username=username, pat=pat)
...
def analyze():
    try:
        ...
        fps = float(request.form.get("fps", 1.0))  # Get FPS with default of 1.0
        ...
        results = analyze_video(
            video_path=filepath, model_config=MODEL_CONFIG, prompt=prompt, fps=fps
        )
...
def chat():
    try:
        ...
        logger.info(
            f"Stored analysis results: {json.dumps(analysis_results, indent=2)}"
        )
...
def analyze_stream():
    try:
        if "video" not in request.files:
            return Response(
                'data: {"error": "No video file provided"}\n\n',
        ...
        if video_file.filename == "":
            return Response(
                'data: {"error": "No video file selected"}\n\n',
                mimetype="text/event-stream",
            )
        ...
        if not username or not pat:
            return Response(
                'data: {"error": "Username and PAT are required"}\n\n',
                mimetype="text/event-stream",
            )
        ...
        if not prompt:
            return Response(
                'data: {"error": "No analysis prompt provided"}\n\n',
                mimetype="text/event-stream",
            )
